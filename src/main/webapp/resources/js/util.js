...
var getEmptyRow = function(){
	return $('<tr class="orderitems-row-feilds"></tr>');
};
...
var getItemsCombobox = function(name, w, cls) {
	var itemsCombo = getCombo(name, w, cls);
	
	$.getJSON("items", function(result) {
        $.each(result, function(index, item) {
            itemsCombo.append('<option value="' + item.id + '">' + item.name + '</option>');
        });
    });
	
	var cont = $("<div style='width: 88%'></div>")
	cont.append(itemsCombo);
...
var addWidgetsToRow = function(widgets) {
	var row = $('<tr class="orderitems-row-feilds"></tr>');
