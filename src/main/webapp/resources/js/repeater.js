$(function() {
	$.widget( "ui.repeater", {
		...
		_create: function() {
			...
			this.mainTable = $('<table cellspacing="15" width="100%"></table>').addClass("order_items_repeater");
			...
			// Widgets
			this.itemName = getItemsCombobox("details_01", 320, "itemName");
			...
			$('.add_ExpenseRow').on("click", function(){
			    if($('.order_items_repeater tr').length <= 6){
	    ...
	    getData: function() {
	    	var data = [];
	    	$('.order_items_repeater tr').each(function(rowIndex, row) {
	    		if (rowIndex > 0){
	    			var item = {};
...
//	    			item.quantity = $(this).find(".amount").val();
	    ...
	    getTableRow: function(){
	        var newRow = $("<tr class='orderitems-row-feilds'></tr>");
	        var td1 = getEmptyColumn();
	        td1.append(getItemsCombobox('details_0' + this.lastChar, 320, "itemName"));
	        newRow.append(td1);
